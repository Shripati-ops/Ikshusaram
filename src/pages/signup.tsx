...
import { useRegister } from "../hooks/registrationHook"
import { useNavigate } from "react-router-dom";
export const Signup = () => {
    const navigate = useNavigate();
    const [email,setEmail] = useState("");
    const [password,setPassword] = useState("");
    const sendUserData = useRegister({email:email,password:password});
    ...
    return (<div style = {{backgroundImage:`url('./132-1326806_download-hd-wallpaper-of-god-balaji-perumal-photos.jpg')`, height:"100vh", width:"100vw", backgroundRepeat:"no-repeat", backgroundSize:"cover"}}>
    ...
    <div className="bg-gray-200 pt-8 pb-16">
      ...
      <div className="w-4/5 mx-auto">
        ...
        <button className="bg-indigo-600 block mx-auto text-white text-sm uppercase rounded shadow-md px-6 py-2" onClick={handleSubmit}>Sign up</button>
      </div>
      <p className="text-sm text-gray-600 text-center"> Already a Member <a className="text-sm text-indigo-700" onClick={navigateToLoginPage}>Sign in</a> </p>
    </div>
    </div>
  </div>)
}